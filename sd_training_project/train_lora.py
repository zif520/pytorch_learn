...
def main():
    parser = argparse.ArgumentParser(description="Stable Diffusion 1.5 LoRA 训练脚本")
    parser.add_argument("--data_dir", type=str, default=None, help="数据目录路径（如不指定则用CIFAR-10）")
    parser.add_argument("--output_dir", type=str, default="output", help="输出目录")
    parser.add_argument("--epochs", type=int, default=10, help="训练轮数")
    parser.add_argument("--batch_size", type=int, default=1, help="批次大小")
    parser.add_argument("--learning_rate", type=float, default=1e-4, help="学习率（LoRA可适当大些）")
    ...
    # 注入LoRA adapter（用peft.LoraConfig）
    ...
    for epoch in range(args.epochs):
        ...
        for batch in progress_bar:
            ...
            if global_step % args.save_steps == 0:
                save_path = output_dir / f"lora_adapter_step_{global_step}"
                # 删除旧的 step 检查点
                for d in output_dir.glob("lora_adapter_step_*"):
                    if d != save_path and d.is_dir():
                        import shutil
                        shutil.rmtree(d)
                unet.save_adapter(save_path, "my_lora")
                print(f"\nLoRA adapter已保存到: {save_path}")
        
        # 每轮结束后保存LoRA adapter
        save_path = output_dir / f"lora_adapter_epoch_{epoch + 1}"
        unet.save_adapter(save_path, "my_lora")
        print(f"LoRA adapter已保存到: {save_path}")
    
    # 保存最终LoRA adapter
    final_save_path = output_dir / "lora_adapter_final"
    unet.save_adapter(final_save_path, "my_lora")
    print(f"\n训练完成！最终LoRA adapter已保存到: {final_save_path}")
